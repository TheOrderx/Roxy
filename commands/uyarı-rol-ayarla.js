exports.run = async (client, message, args) => {
    ...
    try{
        ...
        Data.save((err,doc) => {
            if(err){
                client.Error(message,"uyarı:rol-ayarla",err);
                return;
            }
            ...
            if(doc){
                message.channel.send({ embed: {
                    ...
                    description: `:white_check_mark: Rol başarı ile güncellendi. Uyarılan kullanıcılara **${Role.name}** rolü verilecek.`
        ...
        push.save((err,doc) => {
            ...
            if(doc){
                message.channel.send({ embed: {
                    color: `${client.config.Color.Success}`,
                    description: `:white_check_mark: Rol başarı ile ayarlandı. Uyarılan kullanıcılara **${Role.name}** rolü verilecek.`
                }}).catch(err => { client.Error(message,"uyarı:rol-ayarla",err); });
                return;
            }
